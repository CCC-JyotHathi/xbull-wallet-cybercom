...
export class XdrSignerComponent implements OnInit, OnDestroy {
  ...
  @Output() signingResults: EventEmitter<ISigningResults> =
    new EventEmitter<ISigningResults>();
  @Output() signingResultsHandler?: (data: ISigningResults) => any;

  @Input() from: string | 'wallet' = 'wallet';
  nzDrawerRef: any = null;
  @Input() hideUI = false;
  @Output() errorHandler!: () => void;
  ...
  @Input() set xdr(data: string) {
    try {
      ...
      this.nzMessageService.error(
        'The transaction you are trying to sign is invalid'
      );
      this.onClose();
    }
  }

  pickedAccount$: BehaviorSubject<IWalletsAccount | undefined> =
    new BehaviorSubject<IWalletsAccount | undefined>(undefined);
  ...
  pickedNetworkPassphrase$: BehaviorSubject<
    IHorizonApi['networkPassphrase'] | undefined
  > = new BehaviorSubject<IHorizonApi['networkPassphrase'] | undefined>(
    undefined
  );
  @Input() set pickedNetworkPassphrase(
    networkPassphrase: IHorizonApi['networkPassphrase']
  ) {
    this.pickedNetworkPassphrase$.next(networkPassphrase);
  }

  selectedNetworkPassphrase$: Observable<IHorizonApi['networkPassphrase']> =
    this.pickedNetworkPassphrase$.asObservable().pipe(
      switchMap((pickedNetworkPassphrase) => {
        return !!pickedNetworkPassphrase
          ? of(pickedNetworkPassphrase)
          : this.horizonApisQuery.getSelectedHorizonApi$.pipe(
              pluck('networkPassphrase')
            );
      })
    );
  ...
  operations$: Observable<any> = this.xdrParsed$.pipe(
    map((xdrParse) => xdrParse?.operations || [])
  );

  fee$: Observable<string> = this.xdrParsed$
    .pipe(filter<Transaction>((data) => !!data))
    .pipe(pluck<Transaction, string>('fee'))
    .pipe(map((fee) => new BigNumber(fee).dividedBy('10000000').toString()));
    ...
    .pipe(
      map((operations: any[]) => {
        return operations.map((operation, i) => {
          return this.hostFunctionsService.parseHostFunctionIntoNodeTree(
            operation.function,
            i
          );
        });
  ...
  async onAccepted(): Promise<void> {
    ...
    switch (selectedAccount.type) {
      case 'with_secret_key':
        const passwordAuthTokenActive =
          await this.settingsQuery.passwordAuthTokenActive$
            .pipe(take(1))
            .toPromise();
  ...
  async signWithDeviceAuthToken(
    ...
    this.xdr$
      ...
      .pipe(
        map(([xdr, selectedNetworkPassphrase]) => {
          const secret = this.cryptoService.decryptText(
            selectedAccount.secretKey,
            decryptedPassword
          );
          ...
          const keypair = this.stellarSdkService.keypairFromSecret({
            transaction,
            secret,
          });
      ...
      .subscribe(
        ...
        (error) => {
          this.emitError();
          this.nzMessageService.error(
            `We couldn't sign the transaction, please try again or contact support`
          );
          this.signing$.next(false);
        }
      );
  ...
  async signWithPassword(
    ...
    let password: string;
    const isKeptPasswordActive = await this.settingsQuery.keepPasswordActive$
      .pipe(take(1))
      .toPromise();
    ...
    if (this.ignoreKeptPassword || !isKeptPasswordActive || !savedPassword) {
      ...
      password = await componentRef.password.pipe(take(1)).toPromise();
    ...
    try {
      ...
      const selectedNetworkPassphrase = await this.selectedNetworkPassphrase$
        .pipe(take(1))
        .toPromise();
      ...
      this.emitError();
      this.nzMessageService.error(
        `We couldn't sign the transaction, please check your password is correct`
      );
  ...
  async signWithLedger(selectedAccount: IWalletsAccountLedger): Promise<void> {
    ...
    try {
      const connectedDevices =
        await this.hardwareWalletsService.getConnectedLedgers();
      targetDevice = connectedDevices.find(
        (device) =>
          this.walletsService.generateLedgerWalletId(device) ===
      ...
      this.emitError();
      this.nzMessageService.error(
        `Device not found, please make sure you are using the correct device.`,
        {
          nzDuration: 4000,
        }
      );
    ...
    try {
      transport = await this.hardwareWalletsService.openLedgerConnection(
        targetDevice
      );
    } catch (e: any) {
      this.emitError();
      this.signing$.next(false);
      this.nzMessageService.error(
        `Can\'t connect with the wallet, please make sure your wallet is unlocked and using the Stellar App.`,
        {
          nzDuration: 4000,
        }
      );
    ...
    try {
      this.nzMessageService.info(
        'Check your Ledger and please confirm or cancel the transaction in your device.',
        {
          nzDuration: 4000,
        }
      );

      const passphrase = await this.selectedNetworkPassphrase$
        .pipe(take(1))
        .toPromise();
      ...
      this.nzMessageService.error(
        e?.message ||
          `Make sure your Ledger is unlocked and using the Stellar App. It's possible that your device doesn't support an operation type you're trying to sign`,
        {
          nzDuration: 10000,
        }
      );
  ...
  async signWithTrezor(selectedAccount: IWalletsAccountTrezor): Promise<void> {
    this.signing$.next(true);
    const xdr = await this.xdr$.pipe(take(1)).toPromise();
    const networkPassphrase = await this.selectedNetworkPassphrase$
      .pipe(take(1))
      .toPromise();
    ...
    try {
      ...
      const encodedPublicKey =
        this.stellarSdkService.SDK.StrKey.encodeEd25519PublicKey(
          publicKeyBytes
        );
      ...
      this.signing$.next(false);
      this.nzMessageService.error(
        `Couldn't sign the transaction because there was an unexpected error, please contact support`,
        {
          nzDuration: 4000,
        }
      );
