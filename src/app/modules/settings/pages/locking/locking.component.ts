...
export class LockingComponent implements OnInit, OnDestroy {
  ...
  passwordAuthTokenActiveStatus: Subscription =
    this.settingsQuery.passwordAuthTokenActive$
      .pipe(takeUntil(this.componentDestroyed$))
      .subscribe((status) => {
        this.useDeviceAuthControl.patchValue(status);
      });

  keepPasswordActiveStatus: Subscription =
    this.settingsQuery.keepPasswordActive$
      .pipe(takeUntil(this.componentDestroyed$))
      .subscribe((status) => {
        this.keepPasswordControl.patchValue(status);
      });

  timeoutControlUpdatesSubscription: Subscription =
    this.timeoutMinutesControl.valueChanges
      .pipe(takeUntil(this.componentDestroyed$))
      .subscribe((value) => {
        if (value > 30) {
          this.settingsService.setKeptPasswordTimeout(value);
          this.nzMessageService.success(
            this.translateService.instant('SETTINGS.LOCKING.TIMEOUT_NEVER', {
              value: '',
            })
          );
        } else {
          this.settingsService.setKeptPasswordTimeout(value);
          this.nzMessageService.success(
            this.translateService.instant('SETTINGS.LOCKING.TIMEOUT_UPDATED', {
              value,
            })
          );
        }
  ...
  async requireAuthStart(): Promise<void> {
    if (this.useDeviceAuthControl.value) {
      ...
      this.nzMessageService.success(
        this.translateService.instant('SETTINGS.LOCKING.DEVICE_AUTH_DISABLED')
      );
    } else {
      ...
      componentRef.password
        ...
        .pipe(
          map(([password, globalPasswordHash]) => {
            if (!globalPasswordHash) {
              throw new Error(
                this.translateService.instant('ERROR_MESSAGES.PASSWORD_NOT_SET')
              );
            }
        ...
        .pipe(takeUntil(merge(this.componentDestroyed$, drawerRef.afterClose)))
        .subscribe(
          (encryptResult) => {
            ...
            this.nzMessageService.success(
              this.translateService.instant(
                'SETTINGS.LOCKING.DEVICE_AUTH_ENABLED'
              )
            );
            drawerRef.close();
          },
          (error) => {
            ...
            this.nzMessageService.error(
              error?.message ||
                this.translateService.instant('ERROR_MESSAGES.UNEXPECTED_ERROR')
            );
          }
        );
    }
  ...
  async keepPasswordStart(): Promise<void> {
    if (this.keepPasswordControl.value) {
      ...
      this.nzMessageService.success(
        this.translateService.instant('SETTINGS.LOCKING.KEEP_PASSWORD_DISABLED')
      );
    } else {
      const globalPasswordHash = await this.globalPasswordHash$
        .pipe(take(1))
        .toPromise();
      if (!globalPasswordHash) {
        return;
      ...
      const drawerRef = this.nzDrawerService.create<PasswordModalComponent>({
        ...
        nzContentParams: {
          handlePasswordEvent: (password) => {
            ...
            this.nzMessageService.success(
              this.translateService.instant(
                'SETTINGS.LOCKING.KEEP_PASSWORD_ENABLED'
              )
            );
          },
        },
